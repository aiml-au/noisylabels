...
__all__ = [
    ...
    "CONV_Encoder_CIFAR",
    "CONV_Decoder_CIFAR",
    "CONV_Encoder_CLOTH1M",
    "CONV_Decoder_CLOTH1M",
    "CONV_T_Decoder_CLOTH1M",
    "CONV_Encoder_WEBVISION",
    "CONV_Decoder_WEBVISION",
    "CONV_T_Decoder_WEBVISION",
]
...
class CONV_Decoder_FMNIST(nn.Module):
    def __init__(self, num_classes=2, hidden_dims=[256, 128, 64, 32], z_dim=1):
        ...
        self.final_layer = nn.Sequential(
            ...
            nn.LeakyReLU(),
            nn.Conv2d(hidden_dims[-1], out_channels=1, kernel_size=4),
            nn.Sigmoid(),
        )
...
class CONV_Encoder_ANIMAL(nn.Module):
    def __init__(
        self,
        in_channels=3,
        feature_dim=32,
...
class CONV_Decoder_ANIMAL(nn.Module):
    def __init__(self, num_classes=2, hidden_dims=[256, 128, 64, 32], z_dim=1):
        super().__init__()
        self.decoder_input = nn.Linear(z_dim + num_classes, hidden_dims[0] * 4)
        modules = []
        ...
        self.final_layer = nn.Sequential(
            ...
            nn.LeakyReLU(),
            nn.Conv2d(
                hidden_dims[-1], out_channels=3, kernel_size=3, stride=1, padding=1
            ),
            nn.Sigmoid(),
        )
...
class CONV_T_Decoder_ANIMAL(nn.Module):
    def __init__(
        self,
        in_channels=1,
        feature_dim=64,
...
class CONV_Decoder_WEBVISION(nn.Module):
    def __init__(self, num_classes=2, hidden_dims=[512, 256, 128, 64, 32], z_dim=1):
        super().__init__()
        self.decoder_input = nn.Linear(z_dim + num_classes, hidden_dims[0] * 36)
        modules = []
        for i in range(len(hidden_dims) - 1):
            modules.append(
                nn.Sequential(
...
class T_Decoder(nn.Module):
    def __init__(
        self, feature_dim=2, num_classes=2, num_hidden_layers=1, hidden_size=5
    ):
        super().__init__()
        self.feature_dim = feature_dim

        self.T_fc1 = nn.Linear(feature_dim + num_classes, hidden_size)
        self.T_h_layers = make_hidden_layers(
            num_hidden_layers, hidden_size=hidden_size, prefix="T"
        )
...
class Y_Encoder(nn.Module):
    def __init__(
        ...
        self.y_h_layers = make_hidden_layers(
            num_hidden_layers, hidden_size=hidden_size, prefix="y"
        )
...
class Z_Encoder(nn.Module):
    def __init__(
        self, feature_dim=2, num_classes=2, num_hidden_layers=1, hidden_size=5, z_dim=2
    ):
        super().__init__()
        self.z_fc1 = nn.Linear(feature_dim + num_classes, hidden_size)
        self.z_h_layers = make_hidden_layers(
            num_hidden_layers, hidden_size=hidden_size, prefix="z"
        )
...
class X_Decoder(nn.Module):
    def __init__(
        self, feature_dim=2, num_classes=2, num_hidden_layers=1, hidden_size=5, z_dim=1
    ):
        super().__init__()
        self.recon_fc1 = nn.Linear(z_dim + num_classes, hidden_size)
        self.recon_h_layers = make_hidden_layers(
            num_hidden_layers, hidden_size=hidden_size, prefix="recon"
        )
